...
class HuaweiScanner(ScannerBase):
    def _get_base_urls(self) -> List[str]:
        """生成华为云基础URL"""
        urls = []
        http_url = self.config["CLOUD_TEMPLATES"].get("huawei_http", "").format(
            bucket=self.bucket, region=self.region
        )
        https_url = self.config["CLOUD_TEMPLATES"].get("huawei_https", "").format(
            bucket=self.bucket, region=self.region
        )
        ...
        for base_url in self.base_urls:
            ...
            if resp["status_code"] == 200:
                ...
                if "Effect\": \"Allow" in policy_content and "Principal\": \"*" in policy_content:
                    # 检测是否包含危险操作权限
                    if any(action in policy_content for action in ["obs:PutObject", "obs:DeleteObject", "obs:PutObjectAcl"]):
                        self.results.append({
                            "risk": RiskLevel.CRITICAL2,
                            "msg": f"华为云Bucket存在高危宽松策略（允许匿名写入/删除）: {policy_url}",
                            "url": policy_url
                        })
                    else:
                        self.results.append({
                            "risk": RiskLevel.MEDIUM2,
                            "msg": f"华为云Bucket存在宽松策略（允许匿名读取）: {policy_url}",
                            "url": policy_url
                        })
            ...
            if resp["status_code"] == 200:
                ...
                if "ServerSideEncryptionConfiguration" not in content:
                    self.results.append({
                        ...
                        "msg": f"华为云Bucket未启用服务端加密: {encryption_url}",
                        "url": encryption_url
                    })
                # 检测是否使用KMS但未使用自定义密钥
                    ...
                    self.results.append({
                        ...
                        "msg": f"华为云Bucket使用KMS加密但未使用BYOK（自定义密钥）: {encryption_url}",
                        "url": encryption_url
                    })